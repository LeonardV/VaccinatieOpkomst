...
# datum laatst gewijzigd: 21-07-2021 door LV



# download nieuwste vaccinatie data ---------------------------------------
...
# Wil je enkel onze eigen GGD regio, dan kun je deze uit onderstaande lijst uitzetten.
...
# helper functie: bereken leeftijd o.b.v. dt_vaccinatie en geboortedatum
age_calc <- function(origin, current){
  require(data.table)
...
# set paths: aanpassen naar eigen lokaties
...
# laad HvB vaccinatie data per bu, wk, gm
bu_vac_hvb <- fread(file.path(path_data_hvb, "bu_HVB_MWB_Leonard_2021-07-20.csv"))
wk_vac_hvb <- fread(file.path(path_data_hvb, "wk_HVB_MWB_Leonard_2021-07-20.csv"))
gm_vac_hvb <- fread(file.path(path_data_hvb, "gm_HVB_MWB_Leonard_2021-07-20.csv"))


# CIMS data WB
cims_wb  <- fread(file.path(path_data, "Tabel_vaccinatiegraad_wijk_leeftijdgroep_GGD_West_Brabant_20210719_1045.csv"))
# data HvB
if (length(GM_GGD_regio) == 24) {
  cims_hvb <- fread(file.path(path_data_hvb, "CIMS_HvB_Leonard_2021-07-20.csv"))
  # combineer 
...
# bron: https://github.com/LeonardV/VaccinatieOpkomst/blob/main/data/cbs_pc4_2020_v1.csv
...
# voeg variabele toe of gezet vs gepland
vac_df$gepland <- ifelse(vac_df$vaccinatiestatus == "Vaccinatie gezet", 0, 1)
...
# wel of niet filteren op leeftijd vanaf 15 jaar??? Voorlopig nog wel
vac_df$leeftijd <- age_calc(as.Date(vac_df$geboortedatum, format = "%Y-%m-%d"), 
                            as.Date(vac_df$dt_start_afspraak, format = "%Y-%m-%d"))
# leeftijd vanaf 15 jaar worden meegenomen. De aantallen van 0-14 worden hieronder
# eruit gefilter.
vac_df <- vac_df[which(vac_df$leeftijd >= 15), ]

# subset met geplande afspraken. Voorloopig hebben we enkel de geplande afspraken 
# voor onze eigen ggd regio. 
vac_gepland_df <- subset(vac_df, gepland == 1)
...
vac_gepland_df <- vac_gepland_df[ , c("postcode", "huisnummer")]

names(vac_df)         <- c("PC6", "Huisnummer")
names(vac_gepland_df) <- c("PC6", "Huisnummer")

# verwijder incomplete cases
vac_df         <- vac_df[complete.cases(vac_df), ]
vac_gepland_df <- vac_gepland_df[complete.cases(vac_gepland_df), ]

# voeg bu-, wk-, wk, en pc6-code toe aan vaccinatie data
buwkgm_vac_df         <- merge(vac_df, dfpchn_gmnaam, by = c("PC6", "Huisnummer"), all.x = TRUE)
buwkgm_vac_gepland_df <- merge(vac_gepland_df, dfpchn_gmnaam, by = c("PC6", "Huisnummer"), all.x = TRUE)

# verwijder rijen met onjuiste postcodes
buwkgm_vac_df         <- buwkgm_vac_df[!is.na(buwkgm_vac_df$GM_CODE), ]
buwkgm_vac_gepland_df <- buwkgm_vac_gepland_df[!is.na(buwkgm_vac_gepland_df$GM_CODE), ]
...
wk_vac_gepland <- aggregate(rep(1, nrow(buwkgm_vac_gepland_df)), by = list(buwkgm_vac_gepland_df$WK_CODE), sum)
names(wk_vac) <- c("WK_CODE", "A_VAC")
names(wk_vac_gepland) <- c("WK_CODE", "A_VAC")
...
# NB: dit werkt enkel voor onze GGD regio, NIET voor onze VR!
wk_vac_gepland <- rbind(wk_vac_gepland, wk_vac_hvb)                             #<FIXME>voor hvb hebben we nog geen uitsplitsing naar gpelande afspraken </FIXME>
...
wk_sf_gepland_vac <- merge(wk_sf , wk_vac_gepland , by = "WK_CODE", all.x = TRUE)
...
wk_sf_gepland_vac  <- st_transform(wk_sf_gepland_vac , 4326)
...
wk_sf_vac_gepland_kwb  <- merge(wk_sf_gepland_vac, kwb_df, by.x = "WK_CODE", by.y = "gwb_code_8", all.x = TRUE)
...
wk_sf_vac_gepland_kwb$a_inw_corrected <- wk_sf_vac_gepland_kwb$a_inw - wk_sf_vac_gepland_kwb$a_00_14
...
wk_sf_vac_gepland_kwb$a_inw_corrected[wk_sf_vac_gepland_kwb$a_inw_corrected == 0  ] <- NA
...
wk_sf_vac_gepland_kwb$P_VAC  <- wk_sf_vac_gepland_kwb$A_VAC  / wk_sf_vac_gepland_kwb$a_inw_corrected  * 100
...
cims_df$Wijk_code <- to_number(cims_df$Wijk_code)
# rename var.
names(cims_df) <- c("WK_CODE", "leeftijdscat.", "vaccinatiegraad")

cims_df <- merge(cims_df, wk_sf_vac_gepland_kwb[, c("WK_CODE", "P_VAC")], by = "WK_CODE", all.x = TRUE)
cims_df$geometry <- NULL      
...
# voeg het percentage geplande afspraken toe aan cims data
# Enkel voor onze GGD regio (voorlopig)
if (length(GM_GGD_regio) == 16) {
  cims_df_tot$vaccinatiegraad <- cims_df_tot$vaccinatiegraad + cims_df_tot$P_VAC  
} 

cims_df_tot$P_VAC <- NULL
...
#cbind(wk_sf_vac_kwb[cims.idx, ]$GM_NAAM, wk_sf_vac_kwb[cims.idx, ]$WK_NAAM)
...
#<FIXME>
## beter is om de grand mean te bepalen inclusief de cims data.
...
# In buurten en wijken met minder dan 15 inwoners maskeren we de vaccinatie-aantallen,
# percentages en bins.
bu_inw.idx <- which(bu_sf_vac_kwb$a_inw <= 15)
bu_sf_vac_kwb[bu_inw.idx, ]
bu_sf_vac_kwb$A_VAC[bu_inw.idx] <- NA
bu_sf_vac_kwb$P_VAC[bu_inw.idx] <- NA
bu_sf_vac_kwb$bins[bu_inw.idx ] <- NA

wk_inw.idx <- which(wk_sf_vac_kwb$a_inw <= 15)
wk_sf_vac_kwb[wk_inw.idx, ]
wk_sf_vac_kwb$A_VAC[wk_inw.idx] <- NA
wk_sf_vac_kwb$P_VAC[wk_inw.idx] <- NA
wk_sf_vac_kwb$bins[wk_inw.idx ] <- NA


# save data voor later gebruik --------------------------------------------
bu_tmp <- bu_sf_vac_kwb
wk_tmp <- wk_sf_vac_kwb
gm_tmp <- gm_sf_vac_kwb

bu_tmp$geometry <- NULL
wk_tmp$geometry <- NULL
gm_tmp$geometry <- NULL

write.csv2(bu_tmp, file = file.path("data", paste0(Sys.Date(), "_buurt.csv")))
write.csv2(wk_tmp, file = file.path("data", paste0(Sys.Date(), "_wijk.csv")))
write.csv2(gm_tmp, file = file.path("data", paste0(Sys.Date(), "_gemeente.csv")))
# -------------------------------------------------------------------------
...
# de warnings message kun je negeren.
centroid <- st_centroid(st_union(gm_sf_vac_kwb$geometry))

# locatie ggd wb logo 
img <- "https://www.restriktor.org/ggdWB.png"



## Kaart 1: met de lagen buurt, wijk en gemeenten moet gestuurd worden naar:
# - Maiju
# - Gôkky
# - Gerrie 
# - Arthur
# Note: deze kaart is enkel voor intern gebruik en dus niet bestemd voor bv. gemeenten.

## Kaart 2: met de lagen wijk en gemeente moet ook naar bovenstaande geaddresseerden, 
# maar deze kaart mag wel extern gedeeld worden. De reden is dat de wijk-kaart 
# gecorrigeerd is met CIMS data en de buurtkaart niet. 
...
kaart <- leaflet() %>% 
  ...
  ## dit stuk uit becommentarieren als je de buurt laag wilt aanzetten
  ...
  leafem::addLogo(img, src = "remote", 
                  height = 56, width = 138, alpha = 0.75,
                  url = "https://www.ggdwestbrabant.nl/onderzoek") %>%
  addLayersControl(
    baseGroups = c("Wijk", "Gemeente"), #c("Buurt", "Wijk", "Gemeente")
